...
class SimWriter:
    def __init__(
        ...
        self.out_path = self._init_out_path(
            out_path, outfile_ext, file_suffix=file_suffix
        )
    ...
    def _init_out_path(self, out_path, outfile_ext, file_suffix=""):
        ...
        if out_path.is_dir():
            out_path /= (
                Path(self.OpSimSurvey.opsimdf.attrs["OpSimFile"]).stem
                + file_suffix
                + outfile_ext
            )
        ...
        if self.OpSimSurvey.survey_hosts is not None:
            out_path = out_path.with_stem(
                f"{out_path.stem}_{Path(self.OpSimSurvey.host.attrs['file']).stem}"
            )
        return out_path
...
class SNANA_Simlib(SimWriter):
    ...
    def write_SIMLIB(self, write_batch_size=10, buffer_size=8192):
        ...
        if self.OpSimSurvey.survey_hosts is not None:
            tstart = time.time()
            print(
                f"Writting {len(self.OpSimSurvey.survey_hosts)} hosts in {self.out_path.with_suffix('.HOSTLIB')}"
            )
            self.write_HOSTLIB(
                self.OpSimSurvey.survey_hosts.drop(columns=["ra", "dec"]),
...
class SNSIM_obsfile(SimWriter):
    ...
    def write_survey_file(self, write_survey_conf=True, write_field_map=True):
        if not use_pyarrow:
            raise ModuleNotFoundError("Please install pyarrow to write survey file.")
        ...
        meta = {
            "author_name".encode(): str(self.author_name).encode(),
