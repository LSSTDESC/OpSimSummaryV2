...
def _build_parser():
    ...
    parser.add_argument("--download", "-d", help="Dowload the baseline db file if it did not exist", action="store_true")

    parser.add_argument("--Nfields", "-Nf", help="Number of fields to sample.", default=50_000, type=int)
    parser.add_argument("--hp_nside", "-hpns", help="Nside resolution of healpy pixel.", default=256, type=int)
    parser.add_argument("--min_visits", help="Minimum observation visits", default=500, type=int)
    parser.add_argument("--max_visits", help="Maximum observation visits", default=1e15, type=int)
    parser.add_argument("--wfd_only", help="Only write WFD", action="store_true")
    parser.add_argument("--ddf_only", help="Only write DDF", action="store_true")
    parser.add_argument("--ddf_tags", help="Write DDF tags", action="store_true")
    parser.add_argument("--ddf_nobs_thresh", help="Number of observations threshold between WFD and DDF fields", default=1100, type=int)
    parser.add_argument("--random_seed", "-rs", help="Random seed for survey sampling", default=None, type=int)
    parser.add_argument("--simlib_coadd", help="Run SNANA simlib_coadd.exe. Consider using it if you are simulating DDF.", action="store_true")

    parser.add_argument("--host_file", "-hf", help="absolute path to a host file.", default=None)
    parser.add_argument("--snana_wgtmap", help="SNANA WGTMAP to apply to host.", default=None, type=str)
    parser.add_argument("--author", "-auth", help="Author of the file.", default=None)
    parser.add_argument("--hf_RA_col", "-hfra", help="RA column keys in host file", default="RA_GAL")
    parser.add_argument("--hf_DEC_col", "-hfdec", help="DEC column keys in host file", default="DEC_GAL")
    parser.add_argument("--hf_radec_unit", help="Unit of RA/DEC columns in host file", default="degrees")
    ...
    parser.add_argument("--output_dir", help="Output dir for the SIMLIB. If not specified, uses $SNANA_LSST_ROOT/simlibs.", default=None)

    parser.add_argument("--limit_numpy_threads", "-np_threads", help="Limit the number of threads numpy could use.", default=4, type=int)
    parser.add_argument("--n_cpu", help="Number of cpu to use for matching survey and hosts.", default=10, type=int)
...
def main():
    ...
    _limit_numpy(args.limit_numpy_threads)
    ...
    NOTES["WARNING_NOTICE"] = f"DDF approximately determined by nobs>={args.ddf_nobs_thresh}"
    ...
    if args.wfd_only and args.ddf_only:
        ...
        print(f"Replace min_visits by WFD/DDF threshold: nobs >= {args.ddf_nobs_thresh}")
